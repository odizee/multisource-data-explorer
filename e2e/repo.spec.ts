...
const repo = {
  ...
  owner: { login: 'facebook', avatar_url: 'https://example.com/avatar.png', html_url: '' },
...
test('repo detail renders and handles error', async ({ page }) => {
  ...
  // Error state
  await page.route('https://api.github.com/repos/nope/notfound', async (route) => {
    await route.fulfill({
      status: 404,
      contentType: 'application/json',
      body: JSON.stringify({}),
  ...
  await expect(page.getByText('Failed to load repository details')).toBeVisible();
